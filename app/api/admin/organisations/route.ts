...
export async function POST(request: Request) {
  ...
  try {
    const { userId } = await requirePlatformSuperAdmin()
    ...
    const clerkOrganisation = await clerk.organizations.createOrganization({ name, slug: code.toLowerCase(), createdBy: userId })
    ...
    await clerk.organizations.createOrganizationInvitation({ organizationId: clerkOrganisationId, emailAddress: adminEmail, role: "org:admin", inviterUserId: userId, redirectUrl: new URL("/sign-up", request.url).toString() })
