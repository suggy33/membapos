...
export interface OrderItem { id: string; variantId: string; skuSnapshot: string; descriptionSnapshot: string; qty: number; unitPriceCents: number; discountCents: number; lineTotalCents: number }
export interface Order { id: string; organizationId: string; orderNumber: string; locationId: string; salespersonUserId: string; approvalUserId?: string; status: OrderStatus; items: OrderItem[]; subtotalCents: number; discountTotalCents: number; gstTotalCents: number; totalCents: number; amountPaidCents: number; paymentMethod?: PaymentMethod; createdAt: string; completedAt?: string }
export interface CompleteSaleInput { organizationId: string; locationId: string; orderNumber?: string; items: Array<{ variantId: string; qty: number; discountCents: number }>; paymentMethod: PaymentMethod; amountPaidCents: number; approvalCode?: string }
